...
exports.loginRegister = async (req, res) => {
  try {
    const { emailOrNumber, password, inviteCode } = req.body;

    if (!emailOrNumber || !password) {
      return res
        .status(400)
        .json({ message: "Email/Number and password are required" });
    }

    let user;
    if (/\S+@\S+\.\S+/.test(emailOrNumber)) {
      user = await User.findOne({ email: emailOrNumber.toLowerCase().trim() });
    } else {
      user = await User.findOne({ number: emailOrNumber.trim() });
    }
    ...
    if (!user) {
      ...
      user = new User({
        email: /\S+@\S+\.\S+/.test(emailOrNumber)
          ? emailOrNumber.toLowerCase().trim()
          : null,
        number: !/\S+@\S+\.\S+/.test(emailOrNumber)
          ? emailOrNumber.trim()
          : null,
      ...
      const token = jwt.sign(
        { _id: user._id, email: savedUser.email },
